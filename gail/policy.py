...
from gail.models.resnet import (
...
from habitat_baselines.rl.models.rnn_state_encoder import RNNStateEncoder, build_rnn_state_encoder
...
class ObjectNavGAILNet(Net):
    ...
    def __init__(self, observation_space: Space, model_config: Config,
        ...
        if EpisodicCompassSensor.cls_uuid in observation_space.spaces:
            ...
            self.compass_embedding = nn.Linear(input_compass_dim,
                                               self.compass_embedding_dim)
            rnn_input_size += 32
    ...
    def _extract_sge(self, observations):
        ...
        if "semantic" in observations and "objectgoal" in observations:
            obj_semantic = observations["semantic"].contiguous().flatten(
                start_dim=1)

            if len(observations["objectgoal"].size()) == 3:
                observations["objectgoal"] = observations[
                    "objectgoal"].contiguous().view(
            ...
            goal_visible_area = torch.true_divide(goal_visible_pixels,
                                                  obj_semantic.size(
                                                      -1)).float()
    ...
    def forward(self, observations, rnn_hidden_states, prev_actions, masks):
        ...
        if EpisodicCompassSensor.cls_uuid in observations:
            ...
            if len(obs_compass.size()) == 3:
                obs_compass = obs_compass.contiguous().view(-1,
                                                            obs_compass.size(
                                                                2))
        ...
        if ObjectGoalSensor.cls_uuid in observations:
            ...
            if len(object_goal.size()) == 3:
                object_goal = object_goal.contiguous().view(-1,
                                                            object_goal.size(
                                                                2))
