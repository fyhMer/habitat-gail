...
from habitat_baselines.rl.ddppo.policy import resnet # TODO: check
...
# [Habitat-Web] habitat_baselines.rl.ddppo.policy.resnet_policy
class ResNetEncoder(nn.Module):
    def __init__(
        ...
        if "depth" in observation_space.spaces:
            self._frame_size = tuple(
                observation_space.spaces["depth"].shape[:2])
        ...
        if "semantic" in observation_space.spaces:
            self._frame_size = tuple(
                observation_space.spaces["semantic"].shape[:2])
    ...
    def forward(self, observations: Dict[
        ...
        if self._frame_size == (256, 256):
            ...
            x = F.avg_pool2d(x, (2, 3),
                             padding=(0, 1))  # 240 x 324 -> 120 x 108
        elif self._frame_size == (480, 640):
...
class ResnetSemSeqEncoder(nn.Module):
    def __init__(
        ...
        if not use_goal_seg:
            sem_input_size = 40 + 2
            self.semantic_embedder = nn.Embedding(sem_input_size,
                                                  semantic_embedding_size)
    ...
    def forward(self, observations):
        ...
        if len(obs_semantic.size()) == 5:
            observations["semantic"] = obs_semantic.contiguous().view(
                -1, obs_semantic.size(2), obs_semantic.size(3),
                obs_semantic.size(4)
            )
